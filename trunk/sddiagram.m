...
hold on
% CHANGE NEXT LINE IN SDWDIAGRAM!
...
[h,cord]=circ(0.975,[pi/2 3*pi/2]); delete(h)
cro=rotx(rot2*pi/180);
cord=[cro*[cord zeros(size(cord(:,1)))]']';
...
ang=60;
[h,cord]=circ(cos(ang*pi/180),[-pi/2 pi/2]); delete(h)
eq(6)=plot3(cord(:,1),cord(:,2),ones(size(cord(:,1)))*sin(ang*pi/180));
[h,cord2]=circ(1,[ang 180-ang]*pi/180); delete(h)
eqx=fill3([cord(:,1) ;  ; zeros(size(cord2(:,1)))]',...
	    [cord(:,2) ;  ; cord2(:,1)]',...
	    [ones(size(cord(:,1)))*sin(ang*pi/180) ; ... 
...
% Indicate the angle
proj(5)=plot3([0 0],[0 cos(ang*pi/180)],[0 sin(ang*pi/180)]);
proj(6)=plot3([0 0],[0 -cos(ang*pi/180)],[0 sin(ang*pi/180)]);
[h,cord]=circ(0.5,[pi/2-ang/180*pi/2 pi/2]); delete(h)
arcl(3)=plot3(zeros(size(cord(:,1))),cord(:,1),cord(:,2));
eqh=[];

% Plot polar cap on BOTTOM on extra axis
[ax,axl,loc]=laxis(ah(4),0);
axes(ax)
ang=60;
[h,cord]=circ(cos(ang*pi/180),[-pi/2 pi/2]); delete(h)
hold on
eq(7)=plot3(cord(:,1),cord(:,2),ones(size(cord(:,1)))*sin(ang*pi/180));
[h,cord2]=circ(1,[ang 180-ang]*pi/180); delete(h)
eqx=fill3([cord(:,1) ;  ; zeros(size(cord2(:,1)))]',...
	    [cord(:,2) ;  ; cord2(:,1)]',...
	    [ones(size(cord(:,1)))*sin(ang*pi/180) ; ... 
...
set(eqx,'LineS','--')
% Indicate the angle
proj(7)=plot3([0 0],[0 cos(ang*pi/180)],[0 sin(ang*pi/180)]);
proj(8)=plot3([0 0],[0 -cos(ang*pi/180)],[0 sin(ang*pi/180)]);
[h,cord]=circ(0.5,[pi/2-ang/180*pi/2 pi/2]); delete(h)
%arcl(3)=plot3(zeros(size(cord(:,1))),cord(:,1),cord(:,2));
eqh=[];
viewpars(onax)
...
set([arcl(~~arcl)],'LineW',1,'Color','k')
set(eqh,'LineW',0.5)
set([eq proj],'LineS',':')
set([ah ax],'camerav',5.5) 
...
axes(ah(3))
%l(11)=text(0,0.3,1.1,'R_N');
l(12)=text(0,0.125,0.575,'\Theta');
axes(ah(4))
l(13)=text(0,0.125,0.575,'\Theta');
%l(14)=text(0,0.3,1.1,'R_N');
%l(15)=text(0,0.3,-1.25,'R_S');
